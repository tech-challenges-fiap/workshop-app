# sonarcloud-quality-gate Specification

## Purpose
Wires a non-blocking SonarCloud static analysis scan into `workshop-app`'s CI pipeline, backed by persisted lcov coverage, so quality metrics are tracked without risking the required `test` status check before `SONAR_TOKEN` is configured.
