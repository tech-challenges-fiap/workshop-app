...
- **Action version**: pin `SonarSource/sonarcloud-github-action@v3.1.0`. Newer tags (`v4.0.0`, `v5.0.0`) exist but both are deprecated wrappers that redirect to `SonarSource/sonarqube-scan-action` and print a deprecation warning on every run; `v3.1.0` is the latest non-deprecated release of the `sonarcloud-github-action` itself, matching the plan's explicit instruction to use `SonarSource/sonarcloud-github-action`.
- **Failure isolation**: the Sonar step is a normal (non-`continue-on-error`) step, but it lives in its own step within the `test` job after the tests already ran and reported pass/fail — so a missing `SONAR_TOKEN` only fails the Sonar step's own status check annotation path, not the `lint`, `build`, `container`, or `k8s` jobs, which are separate jobs entirely and run in parallel.

## Risks / Trade-offs

- Until `SONAR_TOKEN` is set via `gh secret set SONAR_TOKEN --repo tech-challenges-fiap/workshop-app`, the `test` job will show the Sonar step failing (the action errors out without a token). This is expected per the closure plan; `lint`, `build`, `container`, and `k8s` remain unaffected since they are independent jobs.
