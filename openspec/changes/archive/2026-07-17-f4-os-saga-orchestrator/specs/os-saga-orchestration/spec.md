...
`workshop-app` SHALL model compensation decisions locally by recording a compensation state and reason, without publishing RabbitMQ messages or invoking external services.
...
- **AND** the compensation reason is persisted for later transport integration
