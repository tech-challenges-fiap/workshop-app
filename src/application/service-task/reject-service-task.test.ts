...
describe("RejectServiceTask", () => {
  it("rejects a pending task", async () => {
    ...
    const useCase = new RejectServiceTask(repository);

    const result = await useCase.execute({ id });

    expect(result.status).toBe(ServiceTaskStatus.REJECTED);
  });
  ...
  it("recalculates work order total when a task is rejected", async () => {
    ...
    const useCase = new RejectServiceTask(serviceTaskRepository, workOrderRepository);

    await useCase.execute({ id: serviceTaskId, workOrderId });

    const updated = await workOrderRepository.findById(workOrderId);
    expect(updated?.toSnapshot().totalAmount).toBe(0);
  });

  it("cancels the work order when all tasks are rejected", async () => {
    const serviceTaskRepository = new InMemoryServiceTaskRepository();
    const workOrderRepository = new InMemoryWorkOrderRepository();
    ...
    const workOrder = WorkOrder.create({
      vehicleId: 1,
      publicToken: "public-token-reject",
      publicTokenExpiresAt: new Date("2024-01-10T12:00:00.000Z"),
      serviceTasks: [
        {
    ...
    const updated = await workOrderRepository.findById(workOrderId);
    expect(updated?.toSnapshot().status).toBe(WorkOrderStatus.CANCELED);
  });
